...
Covers row->PnlEntry building (with graceful skip of malformed rows), the
PostgREST fetch (success + error -> None), and that a failed Telegram send never
leaks the bot token into logs. All network is faked.
"""
...
def test_rows_to_entries_builds_and_skips_malformed():
    rows = [
...
def test_fetch_pnl_rows_success_and_query_shape():
...
def test_fetch_pnl_rows_error_returns_none():
    import requests
...
# --- send_telegram (no token leak) -----------------------------------------

def test_send_telegram_failure_does_not_leak_token(caplog):
    import requests
...
def test_send_telegram_success(caplog):
