...
def test_format_digest_sections_and_totals():
...
def test_format_digest_empty_is_safe():
    text = format_pnl_digest([], asof=ASOF)
    assert "MTD $0.00" in text
    assert "(none)" in text


def test_format_digest_hurdle_line_with_capital():
