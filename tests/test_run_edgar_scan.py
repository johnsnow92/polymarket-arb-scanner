...
field extractors, the best-effort filer/primary-doc resolution, cross-run dedup,
and the enrich->classify->format path. All network is faked — no live SEC calls.
"""
...
from edgar_events import EdgarEventType, format_edgar_alert, scan_filings
...
def test_parse_atom_bad_xml_is_safe():
    assert parse_atom("<not-valid") == []


# --- secondary-fetch helpers ----------------------------------------------

def test_extract_filer_from_index():
...
def test_pick_primary_doc_prefers_largest_non_index():
    blob = {"directory": {"item": [
...
def test_state_roundtrip_and_dedup(tmp_path):
...
def test_save_seen_caps_size(tmp_path):
    path = tmp_path / "state.json"
...
def test_enrich_13e3_needs_no_secondary_fetch():
...
def test_enrich_13d_resolves_activist_filer_and_classifies():
    session = _FakeSession({
...
def test_collect_events_scans_all_forms_and_classifies():
    ...
    assert scanned == 12  # 3 entries × 4 form feeds


def test_build_getcurrent_url():
