    ...
    for pos in open_positions:
        ...
        try:
            if platform == "kalshi" and kalshi_client:
                ...
                try:
                    market_data = predictit_client.fetch_market(int(market_id))
                    if market_data:
                    ...
                    logger.warning("PredictIt settlement check failed for position %s: %s", pos['id'], e)
                ...
                try:
                    market_data = manifold_client.fetch_market(market_id)
                    if market_data and market_data.get("isResolved"):
                        realized = _calc_realized_pnl(db, pos)
                        db.settle_position(pos["id"], realized_pnl=realized, status="settled")
                        settled += 1
                except Exception as e:
